 ...
 * @since       1.6
 */
class SimpleCalendarModelOrganizers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__simplecalendar_organizers').' AS a');
		...
		$query->group('a.id, a.name, a.abbr');
		...
		if (!empty($search))
		{
			if (stripos($search, 'id:') === 0)
			{
				...
				$query->where('a.name LIKE '.$search . ' OR a.abbr LIKE ' . $search);
			}
		}
